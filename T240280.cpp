	...
	for (int k = 1; k < _groupNum + 1; k++) {
		for (int i = _weight; i > -1; i--) {
			for (int m = 1; m < _ccArray[k] + 1; m++) {
				...
				for (int a = m + 1; a < _ccArray[k] + 1; a++) {
					if (i >= _weightList[_myMap[k][m]] + _weightList[_myMap[k][a]]) {
						_dpArray[i] = max(_dpArray[i], _dpArray[i - _weightList[_myMap[k][m]] - _weightList[_myMap[k][a]]] + _valueList[_myMap[k][m]] + _valueList[_myMap[k][a]]);
					}
				}
			}
		}
